...
<?php if(isset($model['error'])) { ?>
    ...
    <?php } ?>
    <form action="/users/tambah" method="POST" class="p-4 p-md-5 border rounded-3 bg-light">
                <h1 class="display-4 fw-bold 1h-1 mb-3 fs-1 text-center mb-5"><?= $model['title'] ?? 'Tambah Data User' ?></h1>
                ...
                <div class="form-floating mb-3">
                    <select name="level" id="level" class="form-select" required>
                        <option value="<?= $_POST['level'] ?? '' ?>">-- Pilih Level --</option>
                ...
                <button class="w-100 button-inverted" type="submit">Simpan</button>
    </form>
    </div>
</div>